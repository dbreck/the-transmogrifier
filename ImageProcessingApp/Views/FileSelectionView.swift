    ...
    VStack(alignment: .leading, spacing: Spacing.sm) {
      ...
      VStack(spacing: Spacing.md) {
        ...
        TransmogrifierButton(
          viewModel.selectedFiles.isEmpty ? "Browse Files" : "Add images or drop more here",
          style: .primary
        ) {
          viewModel.selectFiles()
        }
      }
      .frame(maxWidth: .infinity, minHeight: viewModel.selectedFiles.isEmpty ? 150 : 40)
      .background(.gray800)
      .cornerRadius(8)
      .overlay(
      ...
      .shadow(
        color: isDragTargeted ? Color.blue600.opacity(0.3) : Color.clear, radius: 10, x: 0, y: 0
      )
      .contentShape(Rectangle())
      .onTapGesture { viewModel.selectFiles() }
      .onDrop(of: [UTType.fileURL, UTType.image, UTType.folder], isTargeted: $isDragTargeted) {
        providers in
        var accepted = false
        for p in providers {
  ...
  private func handleDroppedFiles(_ providers: [NSItemProvider]) {
    for provider in providers {
      ...
      if provider.hasItemConformingToTypeIdentifier(UTType.image.identifier) {
        provider.loadInPlaceFileRepresentation(forTypeIdentifier: UTType.image.identifier) {
